# coding:UTF-8
# @Time: 2023/8/14 10:50
# @Author: Lulu Cao
# @File: Tr_GPSRHeat.py
# @Software: PyCharm
from HeatDataset import *
import math
import copy
...
def str2ind(str1,toolbox):
    ...
    sol_1d = parse_expr(str1, evaluate=False)
    ...
    prefix_str = prefix_str.replace("Pow", "pow")
...
def define_gp(X_train,y,d,k=0.4):
    ...
    toolbox.register("evaluate", evalSymbAD, toolbox=toolbox, X_train=X_train, y=y,d=d,k=k)
    ...
    toolbox.register("expr_mut", gp.genFull, min_=1, max_=8)  # 使用完全生成法作为变异算子的基础
    ...
    for ivac, vacind in enumerate(offspring):
        if random.random() < vacb:
            try:
                ...
                offspring[ivac]  = toolbox.population(1)[0]
...
def tr_gp(knowledge_base,toolbox,pset,X_train,y):
    ...
    for g in range(ngen):
        ...
        new_pop[:] = copy.deepcopy(list(map(toolbox.clone, best_inds[:])) + list(map(toolbox.clone, pop[:])) + list(map(toolbox.clone, new_pop[:])))
        ...
        for i, ind in enumerate(offspring):
            if len(ind)>100 or random.random() < prunpb:
                root_index = random.randrange(1, len(ind))
                subtree_slice = ind.searchSubtree(root_index)
        ...
        # 更新种群
        pop[:] = copy.deepcopy(new_pop + offspring)
...
def main(d = 2,k=0.4):
    ...
    if d == 2:
        X_train, y_train = dataset2D(sample=10,k=k)

    elif d == 3:
        X_train, y_train = dataset3D(sample=20,k=k)

    # 定义变量
    toolbox, pset = define_gp(X_train, y_train, d,k)
    knowledge_base = []


    ## TODO 1: 基于1d问题的解，建立知识库

    sol_1d = f'1 / (sqrt({str(math.pi*4*k)} * t)) * exp(-1 * (x1*x1) / ( {str(4*k)} * t))'
    ...
    if d==2:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) >= len(best_ind1):
                knowledge_base.append(copy.deepcopy(toolbox.individual_list(best_ind1[subtree_slice1])))
                knowledge_base.append(copy.deepcopy(toolbox.individual_list(best_ind2[subtree_slice2])))
                # Concatenate the two individuals
                new_expr = toolbox.individual_list(
                    copy.deepcopy(toolbox.individual_from_string('mul')) + copy.deepcopy(oolbox.individual_list(best_ind1[subtree_slice1])[
                                                            :]) + copy.deepcopy(toolbox.individual_list(best_ind2[subtree_slice2])[:]))
                knowledge_base.append(copy.deepcopy(toolbox.clone(new_expr)))
        #
        # sol_1d = f'1 / (({str(math.pi * 4 * k)} * t)) * exp(-1 * (x1*x1+x2*x2) / ( {str(4 * k)} * t))'
        # best_ind1 = str2ind(sol_1d, toolbox)
        # knowledge_base.append(toolbox.clone(best_ind1))
    ...
    if d==3:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1])>= len(best_ind1):
                knowledge_base.append(copy.deepcopy(toolbox.individual_list(best_ind1[subtree_slice1])))
                knowledge_base.append(copy.deepcopy(toolbox.individual_list(best_ind2[subtree_slice2])))
                knowledge_base.append(copy.deepcopy(toolbox.individual_list(best_ind3[subtree_slice3])))
                # Concatenate the two individuals
                new_expr = toolbox.individual_list(copy.deepcopy(toolbox.individual_from_string('mul'))+copy.deepcopy(best_ind1[subtree_slice1][:])+copy.deepcopy(best_ind2[subtree_slice2][:]))
                new_expr = toolbox.individual_list(copy.deepcopy(toolbox.individual_from_string('mul'))+copy.deepcopy(new_expr[:])+copy.deepcopy(best_ind3[subtree_slice3][:]))
                knowledge_base.append(copy.deepcopy(toolbox.clone(new_expr)))
