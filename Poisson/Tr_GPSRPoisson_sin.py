...
def define_gp(X_train,y,d):
    ...
    toolbox.register("expr_mut", gp.genFull, min_=1, max_=4)  # 使用完全生成法作为变异算子的基础
...
def str2ind(str1,toolbox):
    ...
    sol_1d = convert_power_sin(sol_1d)
...
def tr_gp(knowledge_base,toolbox,pset,X_train,y):
    ...
    for g in range(ngen):
        ...
        # 选择下一代的个体
        offspring = toolbox.select(new_pop, popnum)
        # 克隆每个个体，避免修改原始种群
...
def main(d = 1):
    if d == 1:
        X_train, y_train = dataset1D(sample=9)

    elif d == 2:
        X_train, y_train = dataset2D(sample=9)

    elif d == 3:
        X_train, y_train = dataset3D(sample=9)
    ...
    sol_1d = f'sin({str(math.pi)}*x1)'
    best_ind1 = str2ind(sol_1d, toolbox)
    # for i in range(len(best_ind1)):
    #     subtree_slice = best_ind1.searchSubtree(i)
    #     if len(best_ind1[subtree_slice])>2:
    #         knowledge_base.append(toolbox.individual_list(best_ind1[subtree_slice]))
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) >= len(best_ind1):
                ...
                new_expr = toolbox.individual_list(
                    copy.deepcopy(toolbox.individual_from_string('mul')) + copy.deepcopy(toolbox.individual_list(best_ind1[subtree_slice1])[
    ...
    if d==3:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) >= len(best_ind1):
                ...
                # Concatenate the two individuals
                new_expr = toolbox.individual_list(toolbox.individual_from_string('mul')+copy.deepcopy(toolbox.individual_list(best_ind1[subtree_slice1])[:])+copy.deepcopy(toolbox.individual_list(best_ind2[subtree_slice2])[:]))
                new_expr = toolbox.individual_list(toolbox.individual_from_string('mul')+copy.deepcopy(new_expr[:])+
