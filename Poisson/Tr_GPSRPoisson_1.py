...
def tr_gp(knowledge_base,toolbox,pset,X_train,y):
    ...
    for g in range(ngen):
        ...
        for i, ind in enumerate(offspring):
            if len(ind)>90 or random.random() < prunpb:
        ...
        fitness_best[g]=best_fit
        np.savez("poission_f_2d2", fitness_best=fitness_best)
...
def main(d = 2):
    ...
    if d == 2:
        X_train, y_train = dataset2D_poission3(sample=15)

    elif d == 3:
        X_train, y_train = dataset3D_poission4(sample=20)
    ...
    if d==2:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) > 2:
                ...
                new_expr = toolbox.individual_list(
                    copy.deepcopy(toolbox.individual_from_string('mul')) + copy.deepcopy(
    ...
    if d==3:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) > 2:
                ...
                # Concatenate the two individuals
                # new_expr = toolbox.individual_list(toolbox.individual_from_string('mul') + copy.deepcopy(
