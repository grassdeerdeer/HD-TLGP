# coding:UTF-8
# @Time: 2023/8/14 17:40
# @Author: Lulu Cao
# @File: evaluate_sin.py
# @Software: PyCharm
...
def evalSymbAD(individual,toolbox,X_train,y,d):
    ...
    if d == 2:
        ...
        try:
            ...
            print(individual)
        X_pde_train1 = torch.linspace(-1, 10, 10 + 1)
        X_pde_train1 = X_pde_train1.clone().detach().requires_grad_(True)
        X_pde_train2 = torch.linspace(-1, 10, 10 + 1)
        X_pde_train2 = X_pde_train2.clone().detach().requires_grad_(True)
        X_pde_train3 = torch.linspace(-1, 10, 10 + 1)
        X_pde_train3 = X_pde_train3.clone().detach().requires_grad_(True)
        X_pde_train4 = torch.linspace(0, 2, 10 + 1)
...
def torch_Advection2d(expr, X1_train, X2_train,t_train,):
    ...
    # sol_1d = '1 / (4 * 3.14 * 0.4 * t) * exp((-x1 ** 2 - x2 ** 2) / (4 * 0.4 * t))'
    # expr = parse_expr(sol_1d, evaluate=True)
...
#     f_torch = sympytorch.SymPyModule(expressions=[simplified_expr])
#     try:
#         f_x1_x2_t = f_torch(x1=X1_train, x2=X2_train, t=torch.zeros(X1_train.size(), dtype=X1_train.dtype))
#     except:
#         f_x1_x2_t = torch.zeros(X1_train.size(), dtype=X1_train.dtype)
#     du_t_x1_x2 = f_x1_x2_t - torch.sin(X1_train) - torch.sin(X2_train)
#
#     mse = F.mse_loss(du_t_x1_x2, torch.zeros_like(du_t_x1_x2))
#     return mse.item()
#
...
#
#
#     du_t_x1_x2 = f_x1_x2_x3_t - torch.sin(X1_train) - torch.sin(X2_train)- torch.sin(X3_train)
#
#     mse = F.mse_loss(du_t_x1_x2, torch.zeros_like(du_t_x1_x2))
#     return mse.item()
