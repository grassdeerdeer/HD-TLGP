# coding:UTF-8
# @Time: 2023/8/14 15:30
# @Author: Lulu Cao
# @File: Tr_GPSRAdvection_x.py
# @Software: PyCharm
# coding:UTF-8
...
def define_gp(X_train,y,d,k=0.4):
    ...
    toolbox.register("expr_mut", gp.genFull, min_=1, max_=3)  # 使用完全生成法作为变异算子的基础
...
def tr_gp(knowledge_base,toolbox,pset,X_train,y):
    ...
    for g in range(ngen):
        ...
        np.savez("advection_f_2d", fitness_best=fitness_best)
...
def main(d = 2):
    ...
    if d == 2:
        X_train, y_train = dataset2D_advection1(sample=10)

    elif d == 3:
        X_train, y_train = dataset3D_advection2(sample=15)
    ...
    if d==3:
        ...
        for i in range(len(best_ind1)):
            ...
            if len(best_ind1[subtree_slice1]) >= len(best_ind1):
                ...
                new_expr = toolbox.individual_list(
                    toolbox.individual_from_string('mul') + copy.deepcopy(new_expr[:]) +
